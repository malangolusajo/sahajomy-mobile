...
- `core/auth/` — secure tokens, OTP/MFA, refresh serialization, logout, authenticated identity
- `core/workspaces/` — server workspace list, active company/branch/role/permissions, tenant headers, cache isolation
...
- `features/customer/` — dashboard, containers, sea bookings, shipment orders, tracking, Express Air Cargo, China addresses, Agizisha, warehouse access and collection requests
- `features/cargo_admin/` — dashboard, warehouses, containers, sea bookings, receipts/invoices, four-part documentation, manual/scanned intake, shipment orders, FCL, Express Air Cargo, finance, staff/branches, billing, tracking
- `features/sourcing_agent/` — approval state, batches, Agizisha orders/storefront, products, packing lists, financials, containers, sea bookings, Express Air Cargo, tracking
- `features/super_admin/` — users, sourcing agents, cargo administrators, companies, approvals, goods classification, bookings, commission, audit logs, subscriptions/costs, tracking
...
| Cargo Company | Bookings, Containers, Receipts, Account | First permitted cargo operation; no public Home |
...
## Sea and Air booking state machine

Implement one reusable booking shell with cargo-mode adapters rather than separate visual systems.

| State | Progress | Mobile behavior |
|---|---|---|
| `choosingService` | Step 1 active | Show three recommendations. Search and load more only after **Browse all**. |
| `preparingAddress` | Step 1 active/loading | Disable repeated taps and call the provider-scoped address endpoint. Do not reveal the address yet. |
| `editingCargoDetails` | Step 1 checked, Step 2 active | Render the selected operator strip and only Sea- or Air-required fields. |
| `reviewing` | Steps 1–2 checked, Step 3 active | Render a stable draft summary. For Customer Sea, show the shipping mark and link to My China Addresses instead of repeating the full address. No booking POST has occurred yet. |
| `confirming` | Step 3 active/loading | Disable navigation and duplicate confirmation while performing uploads and the booking POST. |
| `confirmed` | All steps checked | Show the authoritative returned reference/status, final summary, China address, View booking, and Book another. |

Router input may include `container_id`, `cargo_admin_id`, or `warehouse_id`. Resolve the identifier against authoritative service data, prepare/reuse the correct address, and restore `editingCargoDetails`. If the identifier is stale or inaccessible, return to `choosingService` with an actionable message.

For 1,000+ operators, keep only the recommendation subset in the initial widget tree. Search should be debounced and pagination/incremental loading must preserve the selected service when more results arrive.
...
6. Warehouse QR access, collection, scan/match/confirm intake, booking linking, loading verification, milestones, and automation controls

...
class CargoContainerDetailPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CargoContainerDetailPage> createState() =>
      _CargoContainerDetailPageState();
}

class _CargoContainerDetailPageState
    extends ConsumerState<CargoContainerDetailPage> {
  ...
  void _reload() {
    ...
    _seaBookings = ref
        .read(cargoOperationsRepositoryProvider)
        .containerSeaBookings(widget.containerId);
  }
        ...
        return ListView(
          ...
          children: [
            CustomerHeroCard(
              ...
              subtitle:
                  '${c['warehouse_origin_name'] ?? 'Origin'} → ${c['warehouse_destination_name'] ?? 'Destination'}',
            ),
            ...
            Row(
              children: [
                ...
                const SizedBox(width: 12),
                Expanded(
                  child: _metric('Price/CBM', '${c['price_per_cbm'] ?? 0}'),
                ),
              ],
            ),
            ...
            _panel([
              _row(
                'ST',
                'Status',
                (c['status'] ?? 'draft').toString().replaceAll('_', ' '),
                brandCoral,
              ),
              ...
              _row(
                'DP',
                'Departure',
                (c['departure_date'] ?? 'TBD').toString().substring(0, 10),
                appMuted,
              ),
              const Divider(height: 1, indent: 60),
              _row(
                'AR',
                'ETA',
                (c['estimated_arrival_date'] ?? 'TBD').toString().substring(
                  0,
                  10,
                ),
                appMuted,
            ...
            FilledButton.icon(
              onPressed: () => context.push(
                '/cargo/containers/${widget.containerId}/consolidated-packing-list',
              ),
              icon: const Icon(Icons.list_alt),
              label: const Text('Consolidated packing list'),
            ),
            const SizedBox(height: 20),
    ...
    child: Row(
      children: [
        Container(
          ...
          child: const Text(
            'RS',
            style: TextStyle(color: brandCoral, fontWeight: FontWeight.w800),
          ),
        ),
        const SizedBox(width: 14),
        Expanded(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 2),
              Text(
                '${r['cbm_booked'] ?? 0} CBM · ${r['goods_status'] ?? 'pending'}',
                style: const TextStyle(color: appMuted, fontSize: 13),
              ),
            ],
          ),
        ...
        CustomerStatusPill(
          label: (r['goods_status'] ?? 'pending').toString().replaceAll(
            '_',
            ' ',
          ),
        ),
  ...
  Future<void> _updateStatus(String status) async {
    try {
      await ref
          .read(cargoOperationsRepositoryProvider)
          .updateContainerStatus(widget.containerId, status: status);
      if (mounted) {
...
class CargoCreateContainerPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<CargoCreateContainerPage> createState() =>
      _CargoCreateContainerPageState();
}

class _CargoCreateContainerPageState
    extends ConsumerState<CargoCreateContainerPage> {
        ...
        children: [
          TextFormField(
            ...
            decoration: const InputDecoration(
              labelText: 'Container size (20ft / 40ft)',
            ),
            validator: (v) =>
          ...
          DropdownButtonFormField<String>(
            initialValue: _originWarehouse.isEmpty ? null : _originWarehouse,
            decoration: const InputDecoration(labelText: 'Origin warehouse'),
            items: _warehouses
                .map(
                  (w) => DropdownMenuItem<String>(
                    value: w['id'].toString(),
                    child: Text(w['name'] ?? 'Warehouse'),
                  ),
                )
                .toList(),
          ...
          TextFormField(
            controller: _maxCbm,
            decoration: const InputDecoration(labelText: 'Max CBM'),
          ...
          TextFormField(
            controller: _price,
            decoration: const InputDecoration(labelText: 'Price per CBM (TZS)'),
            keyboardType: TextInputType.number,
          ),
          const SizedBox(height: 24),
          FilledButton(
            onPressed: _busy ? null : _submit,
            child: Text(_busy ? 'Creating...' : 'Create container'),
          ),
        ],
      ),
    ),

...
class _CargoReceiptsPageState extends ConsumerState<CargoReceiptsPage> {
          ...
          children: [
            Expanded(
              child: FilledButton(
                ...
                style: FilledButton.styleFrom(backgroundColor: _tab == 0 ? brandCoral : appSurface),
                child: Text('Receipts', style: TextStyle(color: _tab == 0 ? Colors.white : appMuted)),
              ),
            ),
            ...
            Expanded(
              child: FilledButton(
                ...
                style: FilledButton.styleFrom(backgroundColor: _tab == 1 ? brandCoral : appSurface),
                child: Text('Invoices', style: TextStyle(color: _tab == 1 ? Colors.white : appMuted)),
              ),
            ),
          ],
        ),
          ...
          builder: (context, snapshot) {
            ...
            if (snapshot.hasError) {
              return CustomerEmptyState(
                ...
                message: 'Could not load ${_tab == 0 ? 'receipts' : 'invoices'}.',
                actionLabel: 'Retry',
                onAction: () => setState(() {
                  _receipts = ref.read(cargoOperationsRepositoryProvider).listReceipts();
                  _invoices = ref.read(cargoOperationsRepositoryProvider).listInvoices();
            ...
            if (list.isEmpty) {
              return CustomerEmptyState(
                icon: _tab == 0 ? Icons.receipt_long_outlined : Icons.description_outlined,
  ...
  Widget _docRow(Map<String, dynamic> d, String badge, int idx) {
    final number = d['receipt_number'] ?? d['invoice_number'] ?? d['reference'] ?? '—';
    final amount = d['amount'] ?? d['total'] ?? 0;
    final currency = d['currency'] ?? 'TZS';
    final status = d['status'] ?? 'issued';
    final date = d['issued_at'] ?? d['created_at'] ?? '—';
    return InkWell(
      onTap: () => context.push('/cargo/receipts/${d['id']}?type=${_tab == 0 ? 'receipt' : 'invoice'}'),
      child: Padding(
        ...
        child: Row(
          children: [
            Container(
              ...
              decoration: BoxDecoration(color: (idx < 2 ? brandCoral : appMuted).withValues(alpha: 0.1), borderRadius: BorderRadius.circular(10)),
              child: Text(badge, style: TextStyle(color: idx < 2 ? brandCoral : appMuted, fontWeight: FontWeight.w800)),
            ),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  Text(number, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 15)),
                  const SizedBox(height: 2),
                  Text('$currency $amount · ${date.toString().substring(0, 10)}', style: const TextStyle(color: appMuted, fontSize: 13)),
                ],
              ),
            ),

...
class _SeaBookingState extends ConsumerState<GuidedSeaBookingPage> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (widget.initialContainerId != null) {
        final selected = services.where(
          (s) => '${s['id']}' == widget.initialContainerId,
        );
        if (selected.isNotEmpty && _available(selected.first)) {
          _choose(selected.first);
        } else {
          setState(
            () => _error = 'This container is no longer available. Choose another sailing.',
          );
        }
      }
    } catch (error) {
  ...
  Future<void> _review() async {
    ...
    try {
      final address = await _repo.prepareSeaAddress(
        containerId: '${_selected!['id']}',
      ...
      if (mounted) {
        setState(
          () => _error = logisticsError(error, 'China forwarding address'),
        );
      }
    } finally {
    ...
    try {
      ...
      final booking = await _repo.confirmSeaBooking(
        ...
        payload: {
          ...
          if (_customer)
            'destination_region': _city.text.trim()
          else
            'destination_city': _city.text.trim(),
          'destination_country': _country.text.trim(),
          if (_customer && _cartons.text.trim().isNotEmpty)
            'carton_count': int.parse(_cartons.text.trim()),
        },
      );
      if (mounted) {
        setState(() {
      ...
      if (mounted) {
        setState(() => _error = logisticsError(error, 'cargo booking'));
      }
    } finally {
      if (mounted) setState(() => _busy = false);
    }
          ...
          children: [
            for (var i = 0; i < 3; i++)
              Expanded(
                child: Semantics(
                  label:
                      '${const ['Service', 'Cargo details', 'Review'][i]}, ${i < _step
                          ? 'complete'
                          : i == _step
                          ? 'current'
                          : 'upcoming'}',
                  child: Column(
                    children: [
                      ...
                      Text(const ['Service', 'Cargo', 'Review'][i]),
                    ],
                  ),
                ),
              ),
  ...
  List<Widget> _servicesView() {
    final matches = _services
        .where(
          (s) => '${_route(s)} ${s['operator']} ${s['container_size']}'
              .toLowerCase()
    ...
    return [
      ...
      if (_loading)
        ...
        OutlinedButton(
          onPressed: _load,
          child: const Text('Reload sea services'),
        )
      ...
      for (final service in matches.take(_limit))
        Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    runSpacing: 8,
                    children: [
                      Text('${service['available_cbm'] ?? '—'} CBM available'),
                      SahajomyStatusPill(
                        label: '${service['status'] ?? 'Status unavailable'}',
                      ),
                    ],
                  ),
      ...
      if (matches.length > _limit)
        TextButton(
          ...
          child: Text(
            _limit == 3 ? 'Browse all sailings' : 'Show more sailings',
          ),
        ),
    ];
  }

  Widget _details() => AbsorbPointer(
    absorbing: _busy,
    child: Form(
      key: _form,
      child: Column(
        ...
        children: [
          LogisticsIntro(
            eyebrow: '${_selected!['operator'] ?? 'Selected sailing'}',
            title: 'Cargo details',
            description: _route(_selected!),
          ),
          TextFormField(
            controller: _volume,
            keyboardType: const TextInputType.numberWithOptions(decimal: true),
            decoration: const InputDecoration(labelText: 'Cargo volume (CBM)'),
            validator: (text) {
              ...
              if (max != null && value > max) {
                return 'Only $max CBM is currently available.';
              }
              if (!_customer && value > 100000) {
                return 'Volume must not exceed 100,000 CBM.';
              }
              return null;
            },
          ),
          ...
          FilledButton(
            ...
            child: Text(
              _busy ? 'Preparing China address…' : 'Review cargo booking',
            ),
          ),
          TextButton(
            onPressed: _busy
            ...
            child: const Text('Change sailing'),
          ),
        ],
      ),
    ),
  ...
  List<Widget> _reviewView() => [
    const LogisticsIntro(
      ...
      description:
          'Check your cargo and destination before confirming the booking.',
    ),
    SahajomyKeyValueList(
      entries: {
        'Cargo company': _selected!['operator'] ?? _selected!['operator_name'],
        'Route': _route(_selected!),
        'Cargo volume': '${_volume.text} CBM',
        'Destination': '${_city.text}, ${_country.text}',
        if (_customer && _cartons.text.isNotEmpty) 'Cartons': _cartons.text,
        'Published rate per CBM':
            '${_selected!['price_per_cbm'] ?? 'Pending'} ${_selected!['currency'] ?? ''}',
      },
    ),
    const SizedBox(height: 24),
    const Text(
    ...
    const SizedBox(height: 8),
    SelectableText(
      '${(_address!['copy'] as Map?)?['shipping_mark'] ?? _address!['shipping_mark'] ?? 'Not provided'}',
    ),
    TextButton(
      onPressed: () => context.push(
        _customer ? '/customer/china-addresses' : '/agent/china-addresses',
      ),
      child: const Text('View China warehouse address'),
    ),
    const SizedBox(height: 20),
    FilledButton(
      onPressed: _busy ? null : _confirm,
      child: Text(_busy ? 'Confirming booking…' : 'Confirm cargo booking'),
    ),
    TextButton(
      onPressed: _busy
          ? null
  ...
  List<Widget> _confirmationView() => [
    ...
    const SizedBox(height: 20),
    const LogisticsIntro(
      title: 'Cargo space booked',
      description: 'Your sea freight booking has been created. Payment and cargo status are shown below.',
    ),
    SahajomyKeyValueList(
      entries: {
        'Booking reference':
            ...
            _booking!['id'],
        'Tracking number': _booking!['tracking_number'],
        'Booked volume': '${_booking!['cbm_booked'] ?? _volume.text} CBM',
        'Freight charge':
            '${_booking!['logistics_charge'] ?? 'Pending'} ${_booking!['currency'] ?? ''}',
        'Payment status': _booking!['payment_status'],
        if (_booking!['goods_status'] != null)
          'Cargo status': _booking!['goods_status'],
      },
    ),
    const SizedBox(height: 24),
    FilledButton(
      onPressed: () => context.go(
        _customer ? '/customer/sea-bookings' : '/agent/sea-bookings',
      ),
      child: const Text('View cargo bookings'),
    ),
    TextButton(
      onPressed: () {
        _volume.clear();
      ...
      child: const Text('Book another sailing'),
    ),
  ];
}
